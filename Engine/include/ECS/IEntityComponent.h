...
struct IEntityComponent {
  ...
  explicit IEntityComponent(bool ShouldStartActive)
      : bIsActive(ShouldStartActive) {};
 ...
 ******************/
// dumb macro that we need to pass commas to another macros param without it
// thinking that it's a new param
#define COMMA ,
...
#define INTERNAL_COMPONENT_BASE_DECLARATION(ComponentName)                     \
  struct ComponentName : public ecs::IEntityComponent {                        \
  private:                                                                     \
    using Super = ecs::IEntityComponent;                                       \
                                                                               \
  public:                                                                      \
    [[nodiscard]] ecs::EntityComponentType GetType() const override {          \
      return ComponentName::GetComponentType();                                \
    }                                                                          \
    [[nodiscard]] static ecs::EntityComponentType GetComponentType() {         \
...
/*
 * Constructor definition macros to allow creating default initialization
 * constructors that support parameters
 */
// Constructor parameters
#define INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_DECLARATION(ParamType,        \
                                                             ParamName)        \
  ParamType ParamName##Input

// Field constructor call
#define INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_INITIALIZER(ParamType,        \
                                                             ParamName)        \
  ParamName(ParamName##Input)

#define INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_INITIALIZER_RAW(ParamName)    \
  ParamName()

// Constructor generation
#define INTERNAL_COMPONENT_CONSTRUCTOR_DECLARATION(                            \
    ComponentName, ParameterDeclarations, ParameterInitializers,               \
    RawInitializers)                                                           \
public:                                                                        \
  ComponentName() : Super(), RawInitializers{};                                \
                                                                               \
  explicit ComponentName(ParameterDeclarations)                                \
      : Super(), ParameterInitializers{};
...
// Single parameter definition for Entity Component
#define DEFINE_COMPONENT_ONE_PARAMETER(ComponentName, ParamType, ParamName)    \
  INTERNAL_COMPONENT_BASE_DECLARATION(ComponentName)                           \
  INTERNAL_COMPONENT_CONSTRUCTOR_DECLARATION(                                  \
      ComponentName,                                                           \
      INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_DECLARATION(ParamType,          \
                                                           ParamName),         \
                                                                               \
      INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_INITIALIZER(ParamType,          \
                                                           ParamName),         \
...
// Double parameter definition for Entity Component
#define DEFINE_COMPONENT_TWO_PARAMETER(ComponentName, ParamType, ParamName,    \
  ...
  INTERNAL_COMPONENT_CONSTRUCTOR_DECLARATION(                                  \
      ...
      INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_DECLARATION(ParamType,          \
                                                           ParamName)          \
          COMMA INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_DECLARATION(          \
              ParamType2, ParamName2),                                         \
                                                                               \
      INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_INITIALIZER(ParamType,          \
                                                           ParamName)          \
          COMMA INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_INITIALIZER(          \
              ParamType2, ParamName2),                                         \
                                                                               \
      INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_INITIALIZER_RAW(ParamName)      \
          COMMA INTERNAL_COMPONENT_CONSTRUCTOR_PARAMETER_INITIALIZER_RAW(      \
              ParamName2))                                                     \
  INTERNAL_COMPONENT_PROPERTY_DECLARATION(ParamType, ParamName)                \
  INTERNAL_COMPONENT_PROPERTY_DECLARATION(ParamType2, ParamName2)              \
  INTERNAL_COMPONENT_END_DECLARATION()

// Triple parameter definition for Entity Component
#define DEFINE_COMPONENT_THREE_PARAMETER(ComponentName, ParamType, ParamName,  \
                                         ParamType2, ParamName2, ParamType3,   \
                                         ParamName3)                           \
  INTERNAL_COMPONENT_BASE_DECLARATION(ComponentName)                           \
  INTERNAL_COMPONENT_CONSTRUCTOR_DECLARATION(                                  \
      ComponentName,                                                           \
...
// Four parameter definition for Entity Component
#define DEFINE_COMPONENT_FOUR_PARAMETER(ComponentName, ParamType, ParamName,                  \
                                        ParamType2, ParamName2, ParamType3,                   \
                                        ParamName3, ParamType4, ParamName4)                   \
                                                                                              \
  INTERNAL_COMPONENT_BASE_DECLARATION(ComponentName)                                          \
